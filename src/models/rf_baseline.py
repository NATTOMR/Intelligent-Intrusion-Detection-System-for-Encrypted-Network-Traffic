...
def load_data(data_dir, sample_frac=0.1):
    ...
    if not files:
        raise FileNotFoundError(
            "No CSV files found in the data directory. Please download the dataset."
        )
    ...
    for file in files:
        ...
        df = (
            pd.read_csv(filepath, sample_frac=sample_frac)
            if hasattr(pd.read_csv, "sample_frac")
            else pd.read_csv(filepath).sample(frac=sample_frac, random_state=42)
        )
        df_list.append(df)
...
def main():
    ...
    parser.add_argument(
        "--data_dir", type=str, default="data/raw", help="Path to raw data directory"
    )
    args = parser.parse_args()

    try:
        df = load_data(
            args.data_dir, sample_frac=0.1
        )  # Use 10% for baseline prototyping
