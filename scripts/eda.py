...
def run_eda(input_path, results_dir):
    ...
    if len(df) > 10000:
        print("Sampling 10,000 rows for EDA to prevent plotting memory issues...")
    ...
    for i, feature in enumerate(top_5_features):
        ...
        sns.histplot(
            data=df, x=feature, hue="Label", bins=50, ax=axes[i], palette="Set2"
        )
...
if __name__ == "__main__":
    ...
    if os.path.exists(input_file):
        ...
        print(
            f"Error: Processed data not found at {input_file}. Please run preprocess.py first."
        )
