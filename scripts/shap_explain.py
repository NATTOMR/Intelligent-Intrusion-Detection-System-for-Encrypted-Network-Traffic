...
def run_shap():
    ...
    plt.title(
        "SHAP Feature Importance — Mean |SHAP| (Top 20)", fontsize=13, fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(results_dir, "shap_summary_bar.png"), dpi=120, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.title(
        "SHAP Beeswarm — Feature Impact Distribution (Top 20)",
        fontsize=13,
        fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(results_dir, "shap_beeswarm.png"), dpi=120, bbox_inches="tight"
    )
    plt.close()
    ...
    plt.title(
        "SHAP Waterfall — Why this packet was flagged as ATTACK",
        fontsize=12,
        fontweight="bold",
    )
    plt.tight_layout()
    plt.savefig(
        os.path.join(results_dir, "shap_waterfall.png"), dpi=120, bbox_inches="tight"
    )
    plt.close()
