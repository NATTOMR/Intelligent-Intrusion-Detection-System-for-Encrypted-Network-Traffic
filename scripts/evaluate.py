...
def run_evaluation():
    ...
    ae_proba = (ae_scores - ae_scores.min()) / (
        ae_scores.max() - ae_scores.min() + 1e-9
    )

    print("Running Hybrid IDS …")
    hybrid_preds = np.copy(xgb_preds)
    benign_idx = np.where(xgb_preds == 0)[0]
    ae_sub_scores = get_ae_scores(
        ae_model, ae_scaler, X_test_full.iloc[benign_idx], ae_features, device
    )
    ...
    ae_sub_norm = (ae_sub_scores - ae_scores.min()) / (
        ae_scores.max() - ae_scores.min() + 1e-9
    )
    ...
    for name, (score, color) in models_roc.items():
        ...
        ax1.plot(
            fpr, tpr, label=f"{name} (AUC={auc_val:.4f})", color=color, linewidth=2
        )
